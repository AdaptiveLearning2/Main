...
| 5 | ~~**`_solve_worker` pays sympy startup inside `SOLVE_TIMEOUT`.**~~ **Done.** The worker signals readiness once sympy is loaded and `_run` times startup and the solve separately. Under CPU saturation: 8/8 where it was 0/8, and 8/8 at 2× oversubscription where the retry had rescued none. See CLAUDE.md for what the two budgets now mean — they are tuned in opposite directions. |
