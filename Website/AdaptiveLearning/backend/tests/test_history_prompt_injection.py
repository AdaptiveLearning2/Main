...
def test_no_spelling_of_a_line_break_survives(breaker):
    ...
    assert breaker.strip() == "" or breaker not in out
    # The general property, not the absence of this one character: whatever the
    # input, the result is one line by the same definition the filter uses.
...
def test_the_generators_receive_flattened_history(monkeypatch):
    ...
    history = decider.get_user_history("kid")
    ...
    decider.question_generation("ordering", "easy", "kid", "5th Grade")
