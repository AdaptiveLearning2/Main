"""Erasure endpoint; the delete itself is tested in scripts/assert_signal_rls.sql."""
...
def parent(monkeypatch):
    ...
    # A real ZoneInfo: a string stub would hide json.dumps raising on one.
...
def test_only_a_linked_parent_can_erase(monkeypatch, parent):
    """Not `_consent_actor`, which admits the student too: nothing undoes this."""
...
def test_an_unconfirmed_request_erases_nothing(monkeypatch, parent):
    """The request carries its own confirmation; a dialog is not auditable."""
    client = _Fake()
...
def test_every_rpc_parameter_survives_json(monkeypatch, parent):
    """postgrest-py json.dumps the params and turns a failure into a bare 500."""
    client = _Fake()
    monkeypatch.setattr(main, "supabase", client)
...
def test_a_storage_failure_is_counted_rather_than_hidden(monkeypatch, parent):
    """The database half has already committed, so a failure is counted, not rolled back."""
...
def test_erasure_is_reported_beside_consent_not_instead_of_it(monkeypatch):
    """Erased and re-consented: the channel is on and the past is gone."""
...
def test_an_unreadable_erasure_table_does_not_blank_the_consent_screen(monkeypatch):
    """Fails open, unlike `_consent()`: it never decides whether anything is recorded."""
    class _Boom:
        def table(self, _n):
