"""The parts of the ONNX export recipe that need no heavy dependencies.

The export itself needs open-rppg, TensorFlow and tf2onnx -- about 2.5 GB, and
absent from CI. But two pieces of `scripts/export_rhythmmamba_onnx.py` are pure
and have known ground truth, so there is no principled reason for them to go
untested:

* the DFT matrices that replace `rfft`/`irfft`, checkable against `numpy.fft`
* `patch()`'s refusal to apply against source it does not recognise, which is
  what stands between a moved upstream and a silently half-patched model

The DFT functions are executed **from the exact string the script injects**,
rather than from a copy pasted here. A copy would drift; this cannot.
"""
...
def test_the_dft_matrices_match_numpy_rfft(L):
    """Odd, even, and the model's actual window length.

    Even lengths matter separately because the Nyquist bin is shared rather
    than mirrored, which is the term the inverse has to weight differently.
    """
...
def test_the_inverse_matrices_reconstruct_the_signal(L):
    """Round-trip, which is what the model actually does: transform, operate in
    the frequency domain, transform back."""
...
def test_the_inverse_matches_numpy_irfft_on_arbitrary_spectra(L):
    """Not just on spectra that came from a real signal. The model multiplies
    the coefficients by learned weights before transforming back, so the inverse
    has to be right for spectra that are not the transform of anything real."""
...
def test_the_matrices_are_float32_so_they_do_not_promote_the_graph():
    """float64 constants would silently upcast every downstream op, which under
    a mixed-precision policy is exactly the mismatch these patches exist to
    avoid."""
...
def test_patching_fails_loudly_when_upstream_has_moved(tmp_path):
    ...
    produces numbers. `open-rppg` moving is expected -- the doc calls the patch
    set an ongoing maintenance cost -- so this is the path that gets taken."""
    ...
    # passed even with this guard removed -- the check fell through to the next
    # one and the test could not tell the difference.
...
def test_patching_refuses_when_the_conv_site_count_is_wrong(tmp_path):
    ...
    quietly rewrite the wrong layer -- which happened once, to BiMamba, whose
    padding differs.
    """
...
def test_patching_twice_is_refused_rather_than_compounded(tmp_path):
    """Applying the rewrites to already-rewritten source would not match, but
    saying so plainly beats a confusing 'could not apply' on the first edit."""
    models = tmp_path / "rppg" / "models.py"
    models.parent.mkdir()
