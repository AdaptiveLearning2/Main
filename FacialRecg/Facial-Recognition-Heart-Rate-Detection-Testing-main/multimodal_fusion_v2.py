...
def build_summary(rows):
    ...
    valid_rows = [
        row
        for row in rows
        if row.get("final", {}).get("state") == "multimodal_state_available"
    ]
    insufficient_rows = [
        row
        for row in rows
        if row.get("final", {}).get("state") == "insufficient_signal"
    ]

    matched_eeg_rows = [
        row
        for row in rows
        if row.get("matchedEegGapSeconds") is not None
    ]
    ...
    return {
        "totalSnapshotCount": len(rows),
        "validFusedSnapshotCount": len(valid_rows),
        "insufficientSignalSnapshotCount": len(insufficient_rows),
        "matchedEegSnapshotCount": len(matched_eeg_rows),
        "averageFinalFocus": (
