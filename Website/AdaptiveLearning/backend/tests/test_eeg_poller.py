...
(sidecar "seat"). Each seat is its own independent physical stream, but
within a *single* seat two different users' pollers both reading it would
...
def test_different_users_same_device_conflict():
    eeg_poller.start(_FakeSupabase(), "user-a", "session-1", "seat-a")
    with pytest.raises(eeg_poller.DeviceClaimedError):
        eeg_poller.start(_FakeSupabase(), "user-b", "session-2", "seat-a")
...
def test_different_users_different_device_both_allowed():
    out_a = eeg_poller.start(_FakeSupabase(), "user-a", "session-1", "seat-a")
    out_b = eeg_poller.start(_FakeSupabase(), "user-b", "session-2", "seat-b")
...
def test_same_user_replaces_own_poller_even_on_different_device():
    eeg_poller.start(_FakeSupabase(), "user-a", "session-1", "seat-a")
    time.sleep(0.05)
    out = eeg_poller.start(_FakeSupabase(), "user-a", "session-2", "seat-b")
    ...
    assert eeg_poller._active["session-2"].device_id == "seat-b"


def test_restarting_same_session_is_a_noop():
    eeg_poller.start(_FakeSupabase(), "user-a", "session-1", "seat-a")
    out = eeg_poller.start(_FakeSupabase(), "user-a", "session-1", "seat-a")
...
def test_released_device_can_be_reclaimed_by_another_user():
    ...
    out = eeg_poller.start(_FakeSupabase(), "user-b", "session-2", "seat-a")
    assert out["running"]
...
def test_is_alive_after_poller_finishes_does_not_raise():
    ...
    """
    p = eeg_poller._Poller(_FakeSupabase(), "user-a", "session-1", "seat-a")
    p.start()
    p.stop()
    p.join(timeout=2.0)
