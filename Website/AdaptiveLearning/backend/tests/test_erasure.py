...
Distinct from withdrawal, which is decided and unchanged: revoking consent
stops future recording and keeps what is stored. This is the separate thing
that issue left open -- a linked parent asking for the history itself to go.

What is asserted here is the endpoint: who may call it, what it refuses, and
that it does not quietly become a second name for withdrawal. The deleting
itself is a Postgres function and is asserted against a real stack in
`scripts/assert_signal_rls.sql`, because a fake client proves nothing about
...
def test_only_a_linked_parent_can_erase(monkeypatch, parent):
    """Not `_consent_actor`, which admits the student. A student may withdraw
    because a parent can undo it; nothing undoes this."""
...
def test_the_student_themselves_cannot_erase(monkeypatch, parent):
    """The sympathetic case, and still refused. Withdrawal and erasure are
    different decisions with different stakes, so they get different gates."""
...
def test_an_unconfirmed_request_erases_nothing(monkeypatch, parent):
    """There is no undo anywhere in the system, so the request carries its own
    confirmation rather than relying on a dialog nobody can audit."""
    client = _Fake()
...
def test_the_refusals_come_before_the_delete_not_after(monkeypatch, parent):
    """Ordering is the whole property. A check that runs after the rows are
    gone is a log line, not a gate."""
    import inspect
...
def test_the_rebuild_uses_the_school_timezone_not_utc(monkeypatch, parent):
    """The rollup rows this rebuilds are bucketed by school day. Rebuilding
    against UTC would leave the survivors bucketed one way and the rest of the
    year the other."""
    client = _Fake()
    monkeypatch.setattr(main, "supabase", client)
...
def test_the_archived_charts_are_removed_too(monkeypatch, parent):
    """The third copy, and the only one outside the database. Leaving them
    means the rows are gone and the pictures of them are not."""
...
def test_a_storage_failure_is_counted_rather_than_hidden(monkeypatch, parent):
    """By then the database half has committed and `chart_paths` no longer
    points at the objects, so they are orphaned but unservable. That is worth
    reporting a number for; it is not worth rolling an erasure back over."""
...
def test_changing_consent_never_erases(monkeypatch):
    """The rule this issue decided in the other direction, pinned from the
    other side. Withdrawal keeps history; if a later change wires a revocation
    to the delete, it has to argue with this."""
    import inspect
...
def test_erasure_is_reported_beside_consent_not_instead_of_it(monkeypatch):
    """A parent who erased and then re-consented has a channel that is on and a
    past that is gone. A payload carrying only one of those is wrong about the
    other."""
...
def test_an_unreadable_erasure_table_does_not_blank_the_consent_screen(monkeypatch):
    """Fails open, unlike `_consent()`. This decides whether a tile says
    "erased" or "no sensor"; it never decides whether anything may be
    recorded."""
    class _Boom:
        def table(self, _n):
